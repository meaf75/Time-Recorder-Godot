# https://github.com/acgc99/Godot-DateChecker

class_name TRDateChecker

## Determines is a year is leap.
static func is_year_leap(year: int) -> bool:
...
static func check_date(year: int, month: int, day: int) -> int:
...
static func check_date_string(date_string: String) -> int:
	var year: int
	var month: int
	var day: int
...
static func check_date_dict(date_dict: Dictionary) -> int:
